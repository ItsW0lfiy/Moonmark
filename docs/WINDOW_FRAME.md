...
Middle-button autoscroll keeps its existing 16 ms motion timer and distance-based speed curve. While active, a lightweight child overlay is centered on the exact click origin and remains stationary as the pointer moves. The original achromatic anchor is painted from Moonmark palette tokens at logical-pixel size, so Qt handles display scaling without copied browser artwork. Escape, a second middle click, a normal click, reload, or session replacement immediately stops the timer and hides the overlay; the former generic vertical-resize cursor is not used.
