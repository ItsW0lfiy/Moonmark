...
Heading anchors remain real `QTextCharFormat` named anchors attached to heading blocks and are indexed directly by anchor name during document construction. Navigation resolves the current cursor geometry, applies a deterministic landing inset, and animates only the scrollbar. Reflow after resize, zoom, or image completion retargets the live semantic anchor from the current visual position, so provisional image geometry does not become a stale destination.
